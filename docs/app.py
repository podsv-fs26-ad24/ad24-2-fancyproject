...
# ---------------------------------------------------
co2_by_year_unit = (
    df.groupby([df["date"].dt.year, "subunit"])["CO2e_RFI2_t"]
    .sum()
...
co2_by_year_unit["pct_change"] = (
    co2_by_year_unit.groupby("subunit")["co2"]
    .pct_change() * 100
)
...
fig_top10_flights.update_traces(textposition="outside")
...
# ---------------------------------------------------
# LAYOUT: KPIs → TOP10 → MAP+TOP10 → LEADERBOARD → TREND → TABELLE
# ---------------------------------------------------

# Top‑10 nebeneinander
col1, col2 = st.columns(2)

with col1:
    st.subheader("Top 10 CO₂-intensivste Strecken")
    st.plotly_chart(fig_top10, use_container_width=True, key="top10_left")

with col2:
    st.subheader("Anzahl Flüge für diese Strecken")
    st.plotly_chart(fig_top10_flights, use_container_width=True, key="topf10_flights")
...
with right:
    st.subheader("Top 10 CO₂-intensivste Strecken")
    st.plotly_chart(fig_top10, use_container_width=True, key="top10_right")
...
# # ---------------------------------------------------
# # LAYOUT: KPIs → MAP + TOP10 → LEADERBOARD → TABELLE
# # ---------------------------------------------------

# # KPIs stehen bereits oben – bleiben unverändert

# # MAP links, TOP10 rechts
# left, right = st.columns([2, 1])
...
# # LEADERBOARD (volle Breite)
# st.subheader("🏆 Leaderboard – CO₂‑Einsparungen")
# st.dataframe(leaderboard)
